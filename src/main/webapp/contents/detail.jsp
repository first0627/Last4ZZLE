...
<body>
	<div id="container" align=center>
		...
		<div style="width: 70%; min-width: 400px; margin-top: 50px;">
			...
			<form action="/insert.review" method="post" id="reviewform2">
				<div class="col-12 d-block d-md-none p-0">
					<div style="float: left; width: 100%; margin-top: 100px;">
						<input type="hidden" name="titlekr" value="${list[0].ktitle}">
						<input type="hidden" name="imgurl" value="${list[0].img}">
						<textarea name="contents" id="contents2" cols="30" rows="7"
							placeholder="짧게라도 좋으니 작품에 대해 감상을 기록해보세요."
...
$("#reviewform2").on("submit",function() {
	   if ($("#contents2").val() == "") {
	      Swal.fire({
	            background: '#121212',
	            html: "내용을 먼저 입력해주세요.",
...
//    })
   ...
   $(".redit2").on("click", function() {
     $(this).parent().siblings("#findrow").find("#ccomment").removeAttr("disabled");
   ...
   $(document).on("click","#rok2",function() {
      Swal.fire({
           background: '#121212',
           html: "수정하시겠어요?",
           color: '#ededed',
            ...
            $.ajax(
                  {
                     url : "/update.review",
                     data : {
                        rseq : $(this).closest("#rrow5").siblings(
                         "#findrow").children(".rseq").val(),
                          contents : $(this).closest("#rrow5").siblings(
                         "#findrow").children("#ccomment").val() 
                     }
                  }).done(function(result) {
                         location.reload();
                   })
